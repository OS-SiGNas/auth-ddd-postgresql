...
export class AuthController implements IAuthController {
	...
	public readonly login: IAuthController["login"] = async ({ body, correlationId }) => {
		try {
			const userDto = await this.#business.login({ login: body, correlationId });
	...
	public readonly activateAccount: IAuthController["activateAccount"] = async ({ body: { token }, correlationId }) => {
		try {
			const user = await this.#business.activateAccount({ token });
