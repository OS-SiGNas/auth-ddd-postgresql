...
export const getLoginSubscriber = (): Handler => {
	...
	const step1: Handler = async (e) => {
		...
		logger.debug("Event: ", e.metadata.correlationId);
	...
	return async (e) => {
		if (DEBUG) logger.info(`Exec: ${e.metadata.correlationId}`);
		...
		try {
			...
			logger.error(`Callback error -> ${e.metadata.emitter} ${e.metadata.correlationId}`);
			errorHandler.catch({ name: action, ticket: e.metadata.correlationId, error });
		}
	};
};
